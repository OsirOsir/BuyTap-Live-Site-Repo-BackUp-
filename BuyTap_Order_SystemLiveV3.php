...
function buytap_render_admin_create_order() {
    ...
    if (!empty($_POST['buytap_create_order_submit']) && check_admin_referer('buytap_admin_create_order')) {
        ...
        if (empty($errors)) {
            ...
            // Deduct from token pool (admins still respect pool)
            if (!buytap_deduct_tokens((int)$amount)) {
                $errors[] = 'Not enough tokens in the pool to create this order.';
            } else {
...
// Settings (defaults)
function buytap_batch_size_default()      { return (int) get_option('buytap_base_batch_size', 5000); }   // per drop
function buytap_drop_interval_default()   { return (int) get_option('buytap_drop_interval_sec', 6*3600); } // 6h
function buytap_soldout_floor_default()   { return (int) get_option('buytap_soldout_floor', 1000); }       // floor

// State
function buytap_public_balance(): int     { return (int) get_option('buytap_public_batch', 0); }
function buytap_set_public_balance($n)    { update_option('buytap_public_batch', max(0, (int)$n)); }

function buytap_next_drop_ts(): int       { return (int) get_option('buytap_next_drop_ts', 0); }
function buytap_set_next_drop_ts($ts)     { update_option('buytap_next_drop_ts', max(0, (int)$ts)); }

// Reserve (already used elsewhere)
function buytap_reserve_balance(): int    { return (int) get_option('buytap_total_tokens', 0); }

// Atomic deduction from PUBLIC batch while keeping a floor (e.g., 1000)
function buytap_deduct_public_batch($amount, $floor = null): bool {
    global $wpdb;
    $amount = (int) $amount;
    ...
    $rows = $wpdb->query(
        $wpdb->prepare(
            ...
            $amount,
            'buytap_public_batch',
            $amount + $floor // ensure remaining >= floor
        )
    );
    if ($rows) {
        wp_cache_delete('buytap_public_batch', 'options');
        return true;
    }
    return false;
}

// Release a new public batch from RESERVE when it's time
function buytap_release_new_batch($size = null, $interval = null) {
    $size     = $size     ?? buytap_batch_size_default();
    $interval = $interval ?? buytap_drop_interval_default();
    ...
    update_option('buytap_total_tokens', $reserve - $batch);
    buytap_set_public_balance(buytap_public_balance() + $batch);

    // schedule next drop
    buytap_set_next_drop_ts(time() + (int)$interval);
}

// Check each minute: if sold out (or time reached), drop new batch
function buytap_maybe_release_batch() {
    $now     = time();
    $next_ts = buytap_next_drop_ts();
    $pub     = buytap_public_balance();
    $floor   = buytap_soldout_floor_default();

    // Release if time passed OR public is below floor (visible sold-out)
    if ($next_ts > 0 && $now < $next_ts && $pub >= $floor) {
        return; // no action needed
    }
    // If no next drop set or time reached or public below floor → release
